package com.dbx.agent.oraclejdbc;
...
class OracleJdbcAgentTest extends JdbcFakeExecutionBehaviorTest {
    @Override
    protected DatabaseAgent createAgent() {
        return new OracleJdbcAgent();
    }
    ...
    void buildUrlUsesExplicitConnectionString() {
        ...
        Assertions.assertEquals("jdbc:oracle:thin:@oracle.example.com:1521:ORCL", OracleJdbcAgent.buildUrl(params));
    }
    ...
    void prepareExecutableSqlKeepsPlsqlObjectTerminator() {
        ...
        Assertions.assertEquals(sql, OracleJdbcAgent.prepareExecutableSql(sql));
    }

    @Test
    void prepareExecutableSqlTrimsPlainStatementTerminator() {
        Assertions.assertEquals("SELECT 1 FROM DUAL", OracleJdbcAgent.prepareExecutableSql("SELECT 1 FROM DUAL;"));
    }
